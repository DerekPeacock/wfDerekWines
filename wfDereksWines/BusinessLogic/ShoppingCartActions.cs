...
namespace wfDereksWines.BusinessLogic
{
    public class ShoppingCartActions : IDisposable
    {
            ...
            using (var db = new WineDbContext())
            {
                try
                {
                    ...
                    foreach (var cartItem in myCart)
                    {
                        ...
                        {
                            if (cartItem.Wine.WineID == CartItemUpdates[i].WineId)
                            {
                                if (CartItemUpdates[i].PurchaseQuantity < 1 || CartItemUpdates[i].RemoveItem == true)
                                {
                                    RemoveItem(cartId, cartItem.WineId);
                                }
                                else
                                {
                                    UpdateItem(cartId, cartItem.WineId, CartItemUpdates[i].PurchaseQuantity);
                                }
                            }
                        }
                    }
        ...
        {
            using (var _db = new Models.WineDbContext())
            {
                try
                {
                    var myItem = (from c in _db.ShoppingCartItems where c.CartId == updateCartID && c.Wine.WineID == updateProductID select c).FirstOrDefault();
                    if (myItem != null)
                    {
